...
create policy if not exists "Admins can view plant credentials"
  on public.plant_credentials for select
  using (get_user_role(auth.uid()) in ('super_admin','admin'));

create policy if not exists "Admins can manage plant credentials"
  on public.plant_credentials for all
  using (get_user_role(auth.uid()) in ('super_admin','admin'))
  with check (get_user_role(auth.uid()) in ('super_admin','admin'));

-- 4) Updated_at trigger
create trigger if not exists update_plant_credentials_updated_at
