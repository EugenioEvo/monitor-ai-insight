...
export const EnergyProductionChart = React.memo(({
  chartData,
  period,
  plant
  ...
  const renderChart = () => {
    if (period === 'DAY') {
      ...
      return <AreaChart data={chartData}>
          ...
          <Area type="monotone" dataKey="energy" stroke="var(--color-energy)" fill="var(--color-energy)" fillOpacity={0.3} />
        </AreaChart>;
    } else {
      ...
      return <BarChart data={chartData}>
          ...
          <XAxis dataKey="date" tick={{
          fontSize: 12
        }} angle={period === 'YEAR' ? -45 : 0} textAnchor={period === 'YEAR' ? "end" : "middle"} height={period === 'YEAR' ? 60 : 40} />
          <YAxis />
          <ChartTooltip content={<ChartTooltipContent />} />
  ...
  return <Card>
      ...
      <CardContent className="bg-slate-50">
        {chartData.length > 0 ? <ChartContainer config={chartConfig} className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              {renderChart()}
            </ResponsiveContainer>
          </ChartContainer> : <div className="flex items-center justify-center h-64 text-muted-foreground">
            <div className="text-center">
              <Calendar className="w-12 h-12 mx-auto mb-4 opacity-50" />
              <p>Nenhum dado de produção disponível</p>
              {plant.monitoring_system === 'manual' && <p className="text-sm mt-2">Configure um sistema de monitoramento para visualizar dados</p>}
            </div>
          </div>}
      </CardContent>
    </Card>;
